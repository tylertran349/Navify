...
class Database:
    ...
    def get_edges_within_polygons(self, polygons):
        ...
        # Create LineStrings for all edges
        edge_line_map = {
            edge: LineString([(edge.node1.x, edge.node1.y), (edge.node2.x, edge.node2.y)]) for edge in outdoor_edges
        }
        ...
        for polygon_coords in polygons:
            ...
            try:
                ...
                polygon = Polygon([(lon, lat) for lat, lon in polygon_coords])  # Ensure (lon, lat) order
            except ValueError as e:
                print(f"Invalid polygon: {polygon_coords}, error: {e}")
                result.append([])
                continue

            inside_or_intersecting_edges = []
            for edge, line in edge_line_map.items():
                if polygon.intersects(line):  # Check if the edge intersects or is within the polygon
                    inside_or_intersecting_edges.append(edge)

            result.append(inside_or_intersecting_edges)
